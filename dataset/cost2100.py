...
class PreFetcher:
    r""" Data pre-fetcher to accelerate the data loading 数据预抓取器来加速数据加载
    """
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):  # 这个代码不知道是干嘛的
...
class Cost2100DataLoader(object):
    ...
    def __init__(self, root, batch_size, num_workers, pin_memory, scenario):
        ...
        # train_dataset、val_dataset、test_dataset = data_test + raw_test
    ...
    def __call__(self):
        ...
        if self.pin_memory is True:  # pin_memory是与gpu有关的
